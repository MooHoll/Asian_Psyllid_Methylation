...
diff_meth_intron_geneIDs <- read_csv("diff_meth_intron_geneIDs.txt", 
                                       col_names = FALSE)
colnames(diff_meth_intron_geneIDs) <- "gene_id"
## -------------------------------------------------------------------------

exon_data <- all_with_meth[all_with_meth$feature=="exon" ,]
intron_data <- all_with_meth[all_with_meth$feature=="intron" ,]
...
intron_data$diff <- "no"
intron_data$diff[intron_data$gene_id %in% diff_meth_intron_geneIDs$gene_id] <- "yes"
#write.table(intron_data, file="all_prom_information.txt", sep="\t",
 #           quote = F, col.names = T, row.names = F)
